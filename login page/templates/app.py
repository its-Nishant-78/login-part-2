...
@app.route('/register',methods=['GETS','POST'])
def register():
    if request.method =='POST':
        ...
        try:
            conn = sqlite3.connect('user_data.db')
            cursor = conn.cursor()
            cursor.execute("INSERT INTO user(username,password,email) VALUE(?,?,?)",
            (userusername, hashed_password, email))

            conn.commit()
            conn.close()
            flash("Registration successfull! you can now log in.","success")
            return redirect(url_for('login'))
        except sqlite3.IndentationError:
